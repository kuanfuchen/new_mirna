...
<script setup>
  ...
  import filter_miRNA_num from '../../utils/filter_miRNA_num.js';
  import { dataService } from '../../service/data_service.js';
  const comSubject$ = new Subject();
  const dispalyCondition = ref('p-value ≤ 0.05, -1 ≤ log2 fold change ≤ 1');
  ...
  dataService.DE_Folder_Info$.pipe(takeUntil(comSubject$), debounceTime(300)).subscribe(async(deFolderData)=>{
    const rna_num = await filter_miRNA_num(0, deFolderData);
  ...
  const changeFeFile = async()=>{
    ...
    if(compareIndex !== -1){
      const rna_num = await filter_miRNA_num(compareIndex, saveDeFolderData);
      await display_filter_RNA_num(rna_num);
    }
  }
  const handleFe_KEGG = async()=>{
    ...
    const tableInfo = [];
    fe_KEGG_TableBody.regulation = '';
    if(compare_de_title.value === '') compare_de_title_group.value[0];
    const selectedTableData = fe_KEGG_Table.data['KEGG'][compare_de_title.value].data;
